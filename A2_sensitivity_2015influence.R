...
GULD.brms.fulltaxa.negbinomA2b <- brm(Count ~
...
STBT.brms.fulltaxa.negbinomA2b <- brm(Count ~
                                       1 + BA + (1 + BA|TaxaGroup) + (1|SampleID) + (1|Month) + (1|Year), data=data_STBTgrouped, 
                                     family = negbinomial(),
                                     control = list(adapt_delta = adapt_delta_value, max_treedepth = max_treedepth_value),
                                     chains = 2, cores = 4, iter = iterations, warmup = burnin, thin = thinning)
#plot(STBT.brms.fulltaxa.negbinomA2b)
summary(STBT.brms.fulltaxa.negbinomA2b)
ranef(STBT.brms.fulltaxa.negbinomA2b)
fixef(STBT.brms.fulltaxa.negbinomA2b)
r2_bayes(STBT.brms.fulltaxa.negbinomA2b, ci = 0.95)

save(STBT.brms.fulltaxa.negbinomA2b, file = "./models/A2_sensitivity/STBT.brms.fulltaxa.negbinomA2b.RData")
